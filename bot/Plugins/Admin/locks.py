...
import bot.Database.locks_sql as sql
from bot import dispatcher, INSPECTOR, LOGGER
...
from bot.Database.approve_sql import is_approved
        ...
        if len(args) >= 1:
            ...
            if ltype in LOCK_TYPES:
                ...
                if conn:
                    ...
                    chat_id = conn
                    chat_name = chat.title
                    text = f"Unlocked {ltype} for everbot in {chat_name}!"
                else:
                    if update.effective_message.chat.type == "private":
                        send_message(
                            update.effective_message,
                    ...
                    chat_id = update.effective_chat.id
                    chat_name = update.effective_message.chat.title
                    text = f"Unlocked {ltype} for everbot!"
